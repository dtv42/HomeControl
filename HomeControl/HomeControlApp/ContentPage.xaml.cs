// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ContentPage.xaml.cs" company="DTV-Online">
...
namespace HomeControlApp.Views
{
    ...
    public sealed partial class ContentPage : Page
    {
        ...
        private bool _initializedControl;
        ...
        {
            Log.Debug("InitializeContent() started.");
            ...
            Log.Debug("InitializeContent() done.");
        }
        ...
        private void InitializeControl()
        {
            ...
            {
                Log.Debug("InitializeControl() started.");

                Log.Debug("InitializeControl() done.");
        ...
        {
            Log.Debug("UpdateControl() started.");
            try
            {
                var json1 = _clientZipato.GetStringAsync("api/zipato/devices").Result;
                var result1 = JsonConvert.DeserializeObject<ZipatoDevices>(json1);
                ...
                if (result1.Status.IsGood)
                {
                    ...
                    if (result2.Count > 7)
                    {
                        ...
                        {
                            Log.Error($"UpdateControl() not OK: Dimmer.");
                        }
                    }
                    else
                    {
                        Log.Error($"UpdateControl() not OK: Scenes.");
                    }
                }
                else
                {
                    Log.Error($"UpdateControl() not OK: {result1.Status.Explanation}.");
                }
            }
            catch (Exception ex)
            {
                Log.Error(ex, $"UpdateControl() exception: {ex.Message}.");
            }
        }
