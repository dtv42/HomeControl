...
namespace HomeControlIoT
{
    ...
    public sealed class StartupTask : IBackgroundTask
    {
        ...
        private ZipatoValues _zipatoValues;
            ...
            try
            {
                ...
                if (CopyAppSettings().Result)
                {
                    ...
                    var configuration = new ConfigurationBuilder()
                        ...
                        .AddUserSecrets<StartupTask>(true)
                    ...
                    // Setting up the InitialState HTTP client.
                    services.AddHttpClient<IInitialStateClient, InitialStateClient>(client =>
                    {
                        client.BaseAddress = new Uri(_settings.BaseAddress);
                        client.Timeout = TimeSpan.FromSeconds(_settings.Timeout);
                        client.DefaultRequestHeaders.TryAddWithoutValidation("x-is-accesskey", _settings.AccessKey);
                        client.DefaultRequestHeaders.TryAddWithoutValidation("x-is-bucketkey", _settings.BucketKey);
                        client.DefaultRequestHeaders.TryAddWithoutValidation("accept-version", "~0");
                    })
                    ...
                    services.AddSingleton<ZipatoValues>();
                    ...
                    _zipatoValues = ActivatorUtilities.CreateInstance<ZipatoValues>(serviceProvider);
        ...
        private async Task SendValuesAsync()
        {
            ...
            _logger?.LogDebug($"Zipato send data {(await _zipatoValues.UpdateValuesAsync() ? "" : "not")} successful.");
        }
