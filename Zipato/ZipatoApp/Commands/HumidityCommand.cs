// --------------------------------------------------------------------------------------------------------------------
// <copyright file="HumidityCommand.cs" company="DTV-Online">
...
namespace ZipatoApp.Commands
{
    ...
    /// </summary>
    [Command(Name = "humidity",
             FullName = "Zipato Humidity Command",
             Description = "Humidity sensor access from Zipato home control.",
             ExtendedHelpText = "\nCopyright (c) 2018 Dr. Peter Trimmel - All rights reserved.")]
    [HelpOption("-?|--help")]
    public class HumidityCommand : BaseCommand<AppSettings>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="HumidityCommand"/> class.
        /// </summary>
        /// <param name="zipato">The Zipato instance.</param>
        /// <param name="logger">The logger instance.</param>
        /// <param name="options">The setting options instance.</param>
        /// <param name="environment">The hosting environment instance.</param>
        public HumidityCommand(IZipato zipato,
                               ILogger<HumidityCommand> logger,
                               IOptions<AppSettings> options,
                               IHostingEnvironment environment)
            : base(logger, options, environment)
        {
            _logger?.LogDebug("HumidityCommand()");
        ...
        public async Task<int> OnExecute(CommandLineApplication app)
        {
            if (CheckOptions(app))
            {
                try
                {
                    ...
                    if (NoParentOptions)
                    {
                        Console.WriteLine($"Humidity Sensors: {JsonConvert.SerializeObject(_zipato.Sensors.HumiditySensors, Formatting.Indented)}");
                        return 0;
                    }
                    else if (OptionIndex)
                    {
                        ...
                        {
                            sensor = _zipato.Sensors.HumiditySensors[index];

                            if (sensor == null)
                            {
                                Console.WriteLine($"Humidity sensor with index '{index}' not found.");
                                return -1;
                            }
                        }
                        else
                        {
                            Console.WriteLine($"Humidity sensor index '{index}' out of bounds (0 - {_zipato.Sensors.HumiditySensors.Count - 1}).");
                            return -1;
                        }
                    }
                    else if (OptionName)
                    {
                        ...
                        sensor = _zipato.Sensors.HumiditySensors.FirstOrDefault(d => d.Name == name);

                        if (sensor == null)
                        {
                            Console.WriteLine($"Humidity sensor with name '{name}' not found.");
                            return -1;
                        }
                    }
                    else if (OptionUuid)
                    {
                        ...
                        sensor = _zipato.Sensors.HumiditySensors.FirstOrDefault(d => d.Uuid == uuid);

                        if (sensor == null)
                        {
                            Console.WriteLine($"Humidity sensor with UUID '{uuid}' not found.");
                            return -1;
                        }
                    }

                    Console.WriteLine($"Humidity sensor: {JsonConvert.SerializeObject(sensor, Formatting.Indented)}");
                }
                catch (Exception ex)
                {
                    _logger?.LogError(ex, $"Exception HumidityCommand.");
                    return -1;
                }
                finally
                {
