// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DevicesCommand.cs" company="DTV-Online">
...
namespace ZipatoApp.Commands
{
    ...
    /// <summary>
    /// Application command "devices".
    /// </summary>
    [Command(Name = "devices",
             FullName = "Zipato Devices Command",
             Description = "Accessing devices from Zipato home control.",
             ExtendedHelpText = "\nCopyright (c) 2018 Dr. Peter Trimmel - All rights reserved.")]
    [HelpOption("-?|--help")]
    [Subcommand("switch", typeof(SwitchCommand))]
    [Subcommand("onoff", typeof(OnOffCommand))]
    [Subcommand("plug", typeof(PlugCommand))]
    [Subcommand("dimmer", typeof(DimmerCommand))]
    [Subcommand("rgb", typeof(RGBCommand))]
    public class DevicesCommand : BaseCommand<AppSettings>
    {
        ...
        [Option("--index <number>", "Using device index.", CommandOptionType.SingleValue, Inherited = true)]
        public int Index { get; set; }

        [Option("--name <string>", "Using device name.", CommandOptionType.SingleValue, Inherited = true)]
        public string Name { get; set; }

        [Uuid]
        [Option("--uuid <uuid>", "Using device uuid.", CommandOptionType.SingleValue, Inherited = true)]
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="DevicesCommand"/> class.
        /// </summary>
        /// <param name="zipato">The Zipato instance.</param>
        /// <param name="logger">The logger instance.</param>
        /// <param name="options">The setting options instance.</param>
        /// <param name="environment">The hosting environment instance.</param>
        public DevicesCommand(IZipato zipato,
                              ILogger<DevicesCommand> logger,
                              IOptions<AppSettings> options,
                              IHostingEnvironment environment)
            : base(logger, options, environment)
        {
            _logger?.LogDebug("DevicesCommand()");
        ...
        public async Task<int> OnExecute(CommandLineApplication app)
        {
            if (CheckOptions(app))
            {
                try
                {
                    ...
                    if (NoOptions)
                    {
                        Console.WriteLine($"Zipato Devices: {JsonConvert.SerializeObject(_zipato.Devices, Formatting.Indented)}");
                    }
                }
                catch (Exception ex)
                {
                    _logger?.LogError(ex, $"Exception DevicesCommand.");
                    return -1;
                }
                finally
                {
