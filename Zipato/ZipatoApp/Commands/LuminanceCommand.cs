// --------------------------------------------------------------------------------------------------------------------
// <copyright file="LuminanceCommand.cs" company="DTV-Online">
...
namespace ZipatoApp.Commands
{
    ...
    /// </summary>
    [Command(Name = "luminance",
             FullName = "Zipato Luminance Command",
             Description = "Luminance sensor access from Zipato home control.",
             ExtendedHelpText = "\nCopyright (c) 2018 Dr. Peter Trimmel - All rights reserved.")]
    [HelpOption("-?|--help")]
    public class LuminanceCommand : BaseCommand<AppSettings>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="LuminanceCommand"/> class.
        /// </summary>
        /// <param name="zipato">The Zipato instance.</param>
        /// <param name="logger">The logger instance.</param>
        /// <param name="options">The setting options instance.</param>
        /// <param name="environment">The hosting environment instance.</param>
        public LuminanceCommand(IZipato zipato,
                                ILogger<LuminanceCommand> logger,
                                IOptions<AppSettings> options,
                                IHostingEnvironment environment)
            : base(logger, options, environment)
        {
            _logger?.LogDebug("LuminanceCommand()");
        ...
        public async Task<int> OnExecute(CommandLineApplication app)
        {
            if (CheckOptions(app))
            {
                try
                {
                    ...
                    LuminanceSensor device = null;

                    if (NoParentOptions)
                    {
                        Console.WriteLine($"Luminance Sensors: {JsonConvert.SerializeObject(_zipato.Sensors.LuminanceSensors, Formatting.Indented)}");
                        return 0;
                    }
                    else if (OptionIndex)
                    {
                        ...
                        {
                            device = _zipato.Sensors.LuminanceSensors[index];

                            if (device == null)
                            {
                                Console.WriteLine($"Luminance sensor with index '{index}' not found.");
                                return -1;
                            }
                        }
                        else
                        {
                            Console.WriteLine($"Luminance sensor index '{index}' out of bounds (0 - {_zipato.Sensors.LuminanceSensors.Count - 1}).");
                            return -1;
                        }
                    }
                    else if (OptionName)
                    {
                        ...
                        device = _zipato.Sensors.LuminanceSensors.FirstOrDefault(d => d.Name == name);

                        if (device == null)
                        {
                            Console.WriteLine($"Luminance sensor with name '{name}' not found.");
                            return -1;
                        }
                    }
                    else if (OptionUuid)
                    {
                        ...
                        device = _zipato.Sensors.LuminanceSensors.FirstOrDefault(d => d.Uuid == uuid);

                        if (device == null)
                        {
                            Console.WriteLine($"Luminance sensor with UUID '{uuid}' not found.");
                            return -1;
                        }
                    }

                    Console.WriteLine($"Luminance sensor: {JsonConvert.SerializeObject(device, Formatting.Indented)}");
                }
                catch (Exception ex)
                {
                    _logger?.LogError(ex, $"Exception LuminanceCommand.");
                    return -1;
                }
                finally
                {
