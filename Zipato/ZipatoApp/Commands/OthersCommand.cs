...
namespace ZipatoApp.Commands
{
    ...
    /// <summary>
    /// Application command "others".
    /// </summary>
    [Command(Name = "others",
             FullName = "Zipato Others Command",
             Description = "Accessing cameras and scenes from Zipato home control.",
             ExtendedHelpText = "\nCopyright (c) 2018 Dr. Peter Trimmel - All rights reserved.")]
    [HelpOption("-?|--help")]
    [Subcommand("scene", typeof(SceneCommand))]
    [Subcommand("camera", typeof(CameraCommand))]
    public class OthersCommand : BaseCommand<AppSettings>
    {
        ...
        [Option("--index <number>", "Using scene index.", CommandOptionType.SingleValue, Inherited = true)]
        public int Index { get; set; }

        [Option("--name <string>", "Using scene name.", CommandOptionType.SingleValue, Inherited = true)]
        public string Name { get; set; }

        [Uuid]
        [Option("--uuid <uuid>", "Using scene uuid.", CommandOptionType.SingleValue, Inherited = true)]
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="OthersCommand"/> class.
        /// </summary>
        /// <param name="zipato">The Zipato instance.</param>
        /// <param name="logger">The logger instance.</param>
        /// <param name="options">The setting options instance.</param>
        /// <param name="environment">The hosting environment instance.</param>
        public OthersCommand(IZipato zipato,
                             ILogger<OthersCommand> logger,
                             IOptions<AppSettings> options,
                             IHostingEnvironment environment)
            : base(logger, options, environment)
        {
            _logger?.LogDebug("ScenesCommand()");
        ...
        public async Task<int> OnExecute(CommandLineApplication app)
        {
            if (CheckOptions(app))
            {
                try
                {
                    ...
                    if (NoOptions)
                    {
                        Console.WriteLine($"Zipato Others: {JsonConvert.SerializeObject(_zipato.Others, Formatting.Indented)}");
                    }
                }
                catch (Exception ex)
                {
                    _logger?.LogError(ex, $"Exception OthersCommand.");
                    return -1;
                }
                finally
                {
